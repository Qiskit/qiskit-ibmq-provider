...
class TestIBMQJob(JobTestCase):
    ...
    @requires_provider
    def test_wait_for_final_state2(self, provider):
        """Test waiting for job to reach final state."""
    ...
    @requires_provider
    def test_wait_for_final_state_status_change(self, provider):
        """Test waiting for job to finish and callback is invoked on status change."""
        ...
        # Put callback data in a dictionary to make it mutable.
        callback_info = {'called': False, 'last data': {}}
        backend = provider.get_backend('ibmq_qasm_simulator')
        qobj = assemble(transpile(self._qc, backend=backend), backend=backend)
        job = backend.run(qobj, validate_qobj=True)

        try:
            job.wait_for_final_state(timeout=30, wait=None, callback=final_state_callback)
            self.assertTrue(job.done())
